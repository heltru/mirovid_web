...
class AttImg
{
    ...
    public $filterResize = ImageInterface::FILTER_SINC;
    ...
    public function preseachImgNew($type,$type_id){ //main
    ...
    private function saveNewImg($type,$type_id){ //add new img
    ...
    private function makeOriginal($img){
        ...
        if ( ! is_dir($dir)) { // new dir
            $successCreateFolder = $fb->createDirectory($dir, 0770);
        ...
        $imgO = clone $imgA;$imgO->id = null;$imgO->isNewRecord = true;
    ...
    public function updateFileImg($img,$type,$type_id)
    {
        ...
        if ( isset($_FILES['Img']) && isset($_FILES['Img']['name'][$img->id]) ){
            ...
            if ($big_img){
                ...
                if (!is_dir('uploads/originals/' . $type . '/' . $type_id)){
                    $fb = BaseFileHelper::createDirectory('uploads/originals/' . $type . '/' . $type_id,0770);
    ...
    private function saveOriginal($id,$type,$type_id){
        ...
        if ( ! is_dir($pathF . $type_id)){ // new dir
            ...
            $successCreateFolder = $fb->createDirectory(  $pathF . $type_id,0770 );
        ...
        if ( $w_src > $h_src ){
            ...
            if ($_D_left_right < 0) { // not valid ratio img
                ...
                $newImg = Image::getImagine()->create(new Box($new_w,$h_src),
                    new \Imagine\Image\Palette\Color\RGB( new \Imagine\Image\Palette\RGB(),[255,255,255], 0 ) );
