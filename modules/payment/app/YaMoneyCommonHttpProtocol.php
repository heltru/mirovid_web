...
class YaMoneyCommonHttpProtocol
{
    ...
    public function processRequest($request) {
        ...
        if ($this->action == 'checkOrder') {
            $this->log("Start checkOrder " . $this->action);
            $response = $this->checkOrder($request);
        } else {
            $this->log("Start paymentAviso " . $this->action);
    ...
    private function checkMD5($request) {
        $str = $request['action'] . ";" .
            ...
            $request['invoiceId'] . ";" . trim($request['customerNumber']) . ";" . $this->settings->shopPassword;
    ...
    private function buildResponse($functionName, $invoiceId, $result_code, $message = null) {
        ...
        try {
            ...
            $response = '<?xml version="1.0" encoding="UTF-8"?><' . $functionName . 'Response performedDatetime="' . $performedDatetime .
                '" code="' . $result_code . '" ' . ($message != null ? 'message="' . $message . '"' : "") . ' invoiceId="' . $invoiceId . '" shopId="' . $this->settings->shopId . '"/>';
            return $response;
        } catch (\Exception $e) {
            $this->log($e);
        }
    ...
    private function log($str) {
        Logs::log('YaMoneyCommonHttpProtocol',$str);
        //$this->log->info($str);
    }
